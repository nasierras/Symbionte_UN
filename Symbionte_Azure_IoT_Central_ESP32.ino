...
// Symbionte headers for functions
#include <stddef.h>
#include <stdio.h>
...
void setupSignalInputReadings() {
  pinMode(MUX01, OUTPUT);  // MUX00
  pinMode(MUX02, OUTPUT);  // MUX01
  pinMode(MUX03, OUTPUT);  // MUX02
  pinMode(MUX04, OUTPUT);  // MUX03
  ads_Amps.begin();
  mpu.begin();
  mpu_init();
}
...
void setup(){
  xTaskCreatePinnedToCore(loop2, "Task_2", 2048, NULL, 1, &Task2, 0);
  Symbiothing.initDevice();
  setupmenuStateMachine();
  setupSignalInputReadings();
...
// loop1 --> Handling MQTT and Internet Functions
void loop(){
